...
use crate::types::{GameRecord, HeroFinal, PlayerRecord, PlayerSummary, UnitSummary};
...
pub fn write_snapshot(
    ...
    for (i, p) in od.players.iter().take(player_count).enumerate() {
        players[i].result = result_str(vread!(p.game_result) as u8).to_string();
        players[i].time_in_upkeep_ms = (0..10_usize).map(|j| vread!(p.time_in_upkeep[j])).collect();
